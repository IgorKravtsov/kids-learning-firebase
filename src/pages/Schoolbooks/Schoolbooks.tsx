...
import { RouteName, FormRoutes } from 'routes'
...
const Schoolbooks: React.FC = (): React.ReactElement => {
  ...
  const getTabValue = (locationPathname: string) => {
    ...
    } else if (locationPathname.endsWith(FormRoutes.THIRD)) {
      return SchoolbookPageTabs.THIRD_FORM
    } else if (locationPathname.endsWith(FormRoutes.FIFTH)) {
      return SchoolbookPageTabs.FIFTH_FORM
  ...
  const booksMenuItems = useMemo(() => subjects.find(subject => subject.link === RouteName.SCHOOL_BOOKS)?.items, [subjects])

  return (
    <Container>
      <Tabs value={value} onChange={handleChange} aria-label='books menu'>
        {booksMenuItems && booksMenuItems.map(({ name, icon, id, link }: MenuItem) => <Tab key={id} label={name} component={Link} to={link} icon={icon} />)}
      </Tabs>
