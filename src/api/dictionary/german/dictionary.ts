...
export const userRef = doc(db, usersCollectionName, auth.currentUser?.uid ?? '')

export const updateTranslationsForWord = async ({ germanWord, translations }: AddTranslationReq) => {
...
export const addTranslations = async ({ germanWord, translations }: AddTranslationReq) => {
  const user = await getUserByEmail(auth.currentUser?.email ?? '')
  const dict = user?.germanDictionary ?? {}
  ...
  await updateDoc(userRef, {
    germanDictionary: {
      ...dict,
      [germanWord]: Array.from(new Set(updatedTranslations)),
    },
  })
}
